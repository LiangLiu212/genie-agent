"""Registry-free per-job records for jobsub-agent.

Each grid submission is one mutable JSON record:
    jobsub-runs/<runtype>-YYYY-MM-DD/<stem>.gridlog

There is **no central registry** (unlike genie-mcp's ~/.genie_mcp_grid_jobs.json):
the jobid `<runtype>-<stem>-<6hex>` decodes itself and `find_record_for_jobid`
globs for the embedded id, mirroring genie-agent/lib/jobs.py. And there is **no
supervisor** (unlike genie-agent's local jobs) — the work runs on the grid, so
status is refreshed on demand by polling jobsub_q (lib/monitor.py) and persisted
...
RUNS_ROOT = _AGENT_ROOT / "jobsub-runs"
...
def new_run_dir(runtype: str, when: datetime | None = None) -> Path:
    """Return (and create) `jobsub-runs/<runtype>-YYYY-MM-DD/`."""
...
def find_record_for_jobid(jobid: str) -> Path:
    """Resolve a jobid to its <stem>.gridlog by globbing jobsub-runs/*/."""
...
def iter_record_paths() -> Iterator[Path]:
    """Yield every <stem>.gridlog under jobsub-runs/, newest dir first."""
