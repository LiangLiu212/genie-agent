...
jobsub_lite is its own python venv (invoked by absolute path), but pixi's
`PYTHONHOME` / `PYTHONPATH` / `PIXI_*` / `CONDA_*` leak into the child and break
it — the grid analog of genie-agent's two-environments problem. We **scrub**
those poisoning vars from a copy of `os.environ` while passing *everything else*
through, so kerberos/token auth (`KRB5CCNAME`, `BEARER_TOKEN_FILE`,
`XDG_RUNTIME_DIR`, `X509_USER_PROXY`, …) survives untouched.

Unlike genie-agent we do **not** snapshot to JSON: auth is live and
per-session, so we scrub at call time. Every `subprocess.run([cfg["jobsub_*"]…])`
passes `env=build_submit_env()`.
"""
