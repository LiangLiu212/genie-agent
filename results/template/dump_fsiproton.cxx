// Dump, for QEL hit-proton events in the Dutta Q^2 window, the two candidate
// post-FSI protons of the v0.2 section-5 comparison:
//   - LEADING  : the highest-|p| final-state (status 1) proton, any ancestry
//                (the spectrometer-like choice used by the ladder/signed p_m)
//   - VERTEX   : the leading status-1 proton DESCENDED from the primary QEL
//                proton (the status-14 hadron-in-nucleus whose mother is the
//                hit nucleon), traced through the GHEP daughter links
...
//   q2,omega,qx,qy,qz,le,lpx,lpy,lpz,ve,vpx,vpy,vpz,same
// (v* = 0 when the primary proton has no final-state proton descendant --
// absorption or charge exchange; same = 1 when LEADING and VERTEX are the
// same GHEP particle.)
...
int main(int argc, char** argv) {
  ...
  fprintf(out, "q2,omega,qx,qy,qz,le,lpx,lpy,lpz,ve,vpx,vpy,vpz,same\n");
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    for (Long64_t ev = 0; ev < n; ++ev) {
      ...
      fprintf(out, "%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%.6g,%d\n",
              q2, probe->E() - fsl->E(), qv.Px(), qv.Py(), qv.Pz(),
              le, lx, ly, lz, ve, vx, vy, vz, (lead >= 0 && lead == vtx) ? 1 : 0);
      ++n_kept;
      mcrec->Clear();
    }
    f->Close();
