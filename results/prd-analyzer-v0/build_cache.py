...
never re-stream. Writes one results/prd-analyzer-v0/cache/<model>.npz per model.
...
SUPERSET_THETA_E_HW, widen the constant and rebuild the superset.
    ...
    pixi run python results/prd-analyzer-v0/build_cache.py --superset # all models, loosest theta_e
    MAX_FILES=2 WORKERS=2 pixi run python results/prd-analyzer-v0/build_cache.py SF   # quick test
