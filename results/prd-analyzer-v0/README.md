...
> **Archived 2026-07-11 as v0** (exploratory phase, ladder steps 1–12). Frozen — new work
> happens in [`../prd-analyzer-v0.1/`](../prd-analyzer-v0.1/README.md). Internal
> `results/prd-analyzer/...` paths were rewritten to `results/prd-analyzer-v0/...` when the
> directory was renamed, so scripts still run in place; the XRootD selection `cache/` moved to
> `../prd-analyzer-v0.1/cache/` (regenerable here via `build_cache*.py` if a v0 re-run is needed).
...
2. **`build_cache.py`** — streams every gst over XRootD (parallel worker processes, `WORKERS`
   ...
   pixi run python results/prd-analyzer-v0/build_cache.py              # all models, branch CUTS
   pixi run python results/prd-analyzer-v0/build_cache.py --superset   # loosest theta_e (+-6)
   pixi run python results/prd-analyzer-v0/build_cache.py UnifiedQEL2024   # just one model
   ```
...
- **`deforest.py`** — numpy port of SIMC's σ_cc1/σ_Mott/Bosted-FF/sigep
  ...
  (`pixi run python results/prd-analyzer-v0/deforest.py`): elastic closure
